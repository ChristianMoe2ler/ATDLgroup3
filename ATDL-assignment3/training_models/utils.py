...
def plot_f1_macro_time(model_name, f1_scores, time_per_epoch, save_path="f1_vs_time_plot.png",
                       data_file="model_data.json"):
    """
    Plot F1 macro scores vs cumulative time for a model.

    Args:
        model_name (str): Name of the model
        f1_scores (list): List of F1 macro scores per epoch
        time_per_epoch (list): List of time taken per epoch
        save_path (str): Path to save the plot
        data_file (str): Path to save the JSON data
    """
    cumulative_time = [0] + [sum(time_per_epoch[:i + 1]) for i in range(len(time_per_epoch))]

    # Modify paths as per original logic
    processed_data_file = data_file + "_f1_macro"
    processed_save_path = save_path + "_f1_macro"

    # Ensure the save path ends with a common image extension
    if not processed_save_path.lower().endswith(('.png', '.jpg', '.jpeg', '.svg')):
        processed_save_path = processed_save_path + ".png"

    # --- Directory creation and file handling for processed_data_file (JSON) ---
    data_file_dir = os.path.dirname(processed_data_file)
    if data_file_dir and not os.path.exists(data_file_dir):
        os.makedirs(data_file_dir, exist_ok=True)
        print(f"Info: Created directory '{data_file_dir}' for F1 macro data file '{processed_data_file}'.")
    ...
    if os.path.exists(processed_data_file):
        if os.path.isfile(processed_data_file):
            try:
                with open(processed_data_file, "r") as f:
                    ...
                    if content.strip():
                        ...
                        all_model_data = {}
                        print(f"Info: F1 macro data JSON file '{processed_data_file}' is empty. Initializing fresh data.")
            except json.JSONDecodeError:
                print(f"Warning: Could not decode JSON from '{processed_data_file}'. Initializing fresh data.")
                all_model_data = {}
            except Exception as e:
            ...
            print(f"Warning: Path '{processed_data_file}' for F1 macro data JSON is a directory. Initializing fresh data.")
            all_model_data = {}
    else:
        print(f"Info: F1 macro data JSON file '{processed_data_file}' not found. Initializing fresh data.")
        all_model_data = {}
    ...
    try:
        with open(processed_data_file, "w") as f:
            json.dump(all_model_data, f, indent=4)
        print(f"Info: F1 macro data for '{model_name}' saved to '{processed_data_file}'.")
    except Exception as e:
        print(f"Error: Could not write F1 macro data to '{processed_data_file}': {e}")

    # --- Plotting F1 Macro vs Time ---
    save_path_dir = os.path.dirname(processed_save_path)
    if save_path_dir and not os.path.exists(save_path_dir):
        os.makedirs(save_path_dir, exist_ok=True)
        print(f"Info: Created directory '{save_path_dir}' for F1 macro plot image '{processed_save_path}'.")

    plt.figure(figsize=(8, 6))
    colors = plt.cm.tab10.colors
    has_data_to_plot = False
    for idx, (name, data) in enumerate(all_model_data.items()):
        if isinstance(data, dict) and "cumulative_time" in data and "f1_macro" in data:
            plt.plot(
                data["cumulative_time"],
                data["f1_macro"],
                label=name,
            ...
            print(f"Warning: Missing data for F1 macro plot for model '{name}'. Skipping.")

    plt.xlabel("Time (seconds)")
    plt.ylabel("F1 Macro Score")
    plt.title("F1 Macro Score vs Time for Multiple Models")
    if has_data_to_plot:
        plt.legend()
    plt.grid(visible=True, which="both", linestyle="--", linewidth=0.5)
    try:
        plt.savefig(processed_save_path)
        print(f"Info: F1 macro plot saved to '{processed_save_path}'.")
    except Exception as e:
        print(f"Error: Could not save F1 macro plot to '{processed_save_path}': {e}")
    plt.close()