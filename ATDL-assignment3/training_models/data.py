...
def load_cifar100(long_tail, batch_size=128):
    ...
    # Handle multi-GPU download: only rank 0 downloads, others wait
    ...
    # Now all ranks can load (download=False since rank 0 already downloaded)
    if rank != 0:
        if long_tail:
            trainset = IMBALANCECIFAR100(root='./data', imb_type="exp", imb_factor=0.01, rand_number=0, train=True, download=False, transform=transform)
        else:
...
def load_cifar10(long_tail, batch_size=128):
    ...
    if rank != 0:
        if long_tail:
            trainset = IMBALANCECIFAR10(root='./data', imb_type="exp", imb_factor=0.01, rand_number=0, train=True, download=False, transform=transform)
        else:
...
def load_medmnist3D(batch_size=128):
    ...
    test_loader = data.DataLoader(dataset=test_dataset, batch_size=batch_size, shuffle=True, num_workers=4)
