#!/usr/bin/env python3
"""
Split dataset into k-folds with specified training size for cross-validation experiments.

This script combines all images from train/ and test/ subdirectories,
then creates k random folds. Each fold has:
- Training set: Specified number of images (e.g., 1, 5, 10)
- Test set: Same size as original test set

Usage:
    python split_dataset_into_folds2.py --dataset bedroom_28 --n_folds 5 --train_size 1 --seed 42
"""
...
def combine_and_split_into_folds(dataset_dir: Path, n_folds: int, train_size: int, seed: int, dry_run: bool = False):
    """
    Combine train/ and test/ images, then create n random train/test splits
    with specified training size and original test size.

    Args:
        dataset_dir: Directory containing train/ and test/ subdirectories
        n_folds: Number of random splits to create
        train_size: Number of images in each training fold
    ...
    for fold_idx in range(n_folds):
        ...
        # Split: first train_size for training, next original_test_count for testing
        train_fold_images = shuffled_images[:train_size]
        test_fold_images = shuffled_images[train_size:train_size + original_test_count]
        ...
        if dry_run:
            ...
            print(f"    - train/: {len(train_fold_images)} images")
            print(f"    - test/: {len(test_fold_images)} images")
        else:
            ...
            for img_path in train_fold_images:
            ...
            # Copy test images
            for img_path in test_fold_images:
                label_path = img_path.with_suffix('.npy')
            ...
            print(f"    - train/: {len(train_fold_images)} images, {train_annotations} annotations")
            print(f"    - test/: {len(test_fold_images)} images, {test_annotations} annotations")
...
def main():
    parser = argparse.ArgumentParser(
        description='Combine train/test data and split into k-folds with specified training size'
    )
    parser.add_argument(
        '--dataset',
        type=str,
    ...
    parser.add_argument(
        ...
        type=int,
        required=True,
        help='Number of training images per fold (e.g., 1, 5, 10)'
    )
    parser.add_argument(
        '--seed',
        type=int,
    ...
    print(f"="*60)
    print(f"K-Fold Cross-Validation Dataset Split (Custom Training Size)")
    print(f"="*60)
    ...
    combine_and_split_into_folds(dataset_dir, args.n_folds, args.train_size, args.seed, args.dry_run)
    ...
    if not args.dry_run:
        ...
        for i in range(args.n_folds):
            print(f"  - fold_{i+1}/ ({args.train_size} train, original test size)")
