...
SEED=42
DRY_RUN=false
# MLP Architecture Variant (choose one: baseline, wider, deeper)
MLP_TYPE="deeper"
...
echo "Combining train/test data and splitting ${DATASET} into ${N_FOLDS} folds with seed ${SEED}..."
python3 ../split_dataset_into_folds.py \
    --dataset "${DATASET}" \
    --n_folds ${N_FOLDS} \
    --seed ${SEED} \
    --base_dir datasets
...
#####################################
# 4.5. modify the pixel classifier
#####################################
echo ""
echo "=========================================="
echo "  Modifying MLP Architecture: ${MLP_TYPE}"
echo "=========================================="

python3 ../modify_classifier.py "${MLP_TYPE}"
...
################################################################################
# 6. Restore Original Classifier
################################################################################

if [ -f "src/pixel_classifier.py.backup" ]; then
    echo "Restoring original pixel_classifier.py..."
    mv src/pixel_classifier.py.backup src/pixel_classifier.py
fi
...
echo ""
echo "NOTE: Model files were overwritten for each fold."
